...
class Chest {
  ...
  /**
   * Validates that a chest key file exists.
   *
   * @param {string} keyFilePath - path to the encryption key.
   *
   * @returns {boolean} true or false.
   */
  static hasAKeyFile(keyFilePath) {
    return fs.existsSync(path.join(keyFilePath, '.chest_key'));
  }

  /**
   * Validates that a chest file exists.
   *
   * @param {string} secretsPath - path to the secret data file.
   *
   * @returns {boolean} true or false.
   */
  static hasASecretsFile(secretsPath) {
    return ['.yml', '.json', '.yaml'].some(
      ext => fs.existsSync(
        path.join(secretsPath, this.name.toLowerCase() + ext)));
  }
